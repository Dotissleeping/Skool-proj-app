// src/screens/AddScheduleScreen.jsx
//
// Add-class form: subject name, multi-day selection, start/end time.
// Validates input, checks for time conflicts, and saves via useSchedule.
...
export default function AddScheduleScreen({ navigation }) {
  ...
  const { addClass, checkConflicts } = useSchedule();

  const [subjectName, setSubjectName] = useState('');
  const [days, setDays] = useState([]);
  const [startTime, setStartTime] = useState(480); // 08:00 AM
  ...
  function handleSave() {
    ...
    const conflicts = checkConflicts({ days, startTime, endTime });
    if (conflicts.length > 0) {
      ...
      Alert.alert(
        ...
        [
          ...
          { text: 'Add Anyway', style: 'destructive', onPress: saveClass },
        ]
      );
      return;
    }
  ...
  function saveClass() {
    addClass({ subjectName: subjectName.trim(), days, startTime, endTime });
    navigation.goBack();
  }

  return (
    <View style={[styles.flex, { backgroundColor: theme.colors.background }]}>
      <AppHeader title="Add Schedule" onBack={() => navigation.goBack()} />
